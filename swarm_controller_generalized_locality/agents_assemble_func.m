function [ percentComplete ] = agents_assemble_func()
...
controlFunction = @(f) agentController(f,P);
...
for j = 1:length(contF)
    contF = controlFunction(forceFunction(df(j)))
end
hold on
%plot(d,locF,'b')
plot(df,contF,'r')
...
while percentComplete < 1
    ...
    [Px,Py,Vx,Vy,~,takenAgentsTargets] = updateGravitational(Px,Py,Vx,Vy,Tx,Ty,b,forceFunction,controlFunction,agentSize,takenAgentsTargets);
